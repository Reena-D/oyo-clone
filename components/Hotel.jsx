...
const Hotel = ({ e }) => {
  return (
    <div className="border-2 border-red-500 rounded-lg h-100 w-full mb-5 p-5">
      <div className="flex">
        <Image src={e?.banner} alt="hotel" width={200} height={200} className="w-96 h-60 mr-3"/>
      <div className="flex flex-col justify-between">
      {
        e ? e.gallery?.map((ele)=>{
         ...
         <Image 
           key={ele}
           src={ele}
       alt="hotel" width={200} height={200} className="w-28 h-16 object-cover"/>
...

  {
    e ? e.facilities?.map((ele) =>{
      return (
        ...
        <Link href={`/hotels/${e?._id}`} className='text-xl font-bold text-red-600 ml-10'>View Details</Link>
